...
python3 $SCRIPTS_PATH/classic-scrape-recursive.py "$URL/$LATEST_FOLDER" | grep tar |  grep rc | sed 's/.*rc.*//g' | grep $PACKAGE_PREFIX &> /dev/null
if [[ $? == 1 ]]; then
  LATEST_FOLDER=$(python3 $SCRIPTS_PATH/classic-scrape-recursive.py $URL | grep [0-9] | grep '\.' | sort -rV | sed '1d' | head -1)
fi
LATEST_VERSION=$(python3 $SCRIPTS_PATH/classic-scrape-recursive.py "$URL/$LATEST_FOLDER" | grep tar |  grep $PACKAGE_PREFIX | sed 's/.*-//g' | grep -E -o "[0-9]+\.[0-9]+\.[0-9]+" | sort -rV | head -1 )
echo $LATEST_VERSION
